...
namespace SDV_Realty_Core.Framework.ServiceProviders.ModMechanics
{
    ...
    internal class GridManager : IGridManager
    {
        ...
        internal override int AddMapToGrid(string expansionName, int gridId)
        {
            ...
            //
            int returnId = -1;
            FarmExpansionLocation oBase = modDataService.farmExpansions[expansionName];
            if (gridId > -1)
            {
                //addition from MP Server
                //
                if (MapGrid.Where(p => p.Value == expansionName).Any())
                {
                    ...
                    if (localId != gridId)
                    {
                        // synch error fix local to match server
                        //
                        MapGrid.Remove(localId);
                    }
                }
                else if (oBase.GridId != -1 && oBase.GridId != gridId && MapGrid.ContainsKey(oBase.GridId))
                {
                    // snych issue
                    MapGrid.Remove(oBase.GridId);
                }
                MapGrid.Add(gridId, expansionName);
                returnId = gridId;
            }
            else
            {
                if (MapGrid.Where(p => p.Value == expansionName).Any())
                {
                    ...
                    //  already added, return existing id
                    //
                    returnId = MapGrid.Where(p => p.Value == expansionName).First().Key;
                }
                else
                {
                    //
                    ...
                    if (oBase.GridId > -1)
                    {
                        if (MapGrid.ContainsKey(oBase.GridId))
                        {
                            ...
                            if (oBase.GridId > -1)
                            {
                                MapGrid.Add(oBase.GridId, expansionName);
