...
namespace SDV_Realty_Core.Framework.ServiceProviders.ModMechanics
{
    internal class ExitsService : IExitsService
    {
        ...
        internal override void Initialize(ILoggerService logger, object[] args)
        {
            ...
            _contentPackService = (IContentPackService)args[0];
            _modDataService = (IModDataService)args[1];
            _mapUtilities = (IMapUtilities)args[2];
            _utilitiesService = (IUtilitiesService)args[3];
        ...
        {
            if (!_modDataService.validContents.Any())
            {
            ...
            {
                if (oExitPatch.Item2.Key == "0" && _modDataService.Config.useNorthWestEntrance || oExitPatch.Item2.Key == "1" && _modDataService.Config.useSouthWestEntrance)
                {
                    ...
                    _mapUtilities.PatchInMap(glExitFarm, mExitPatchMap, new Point(oExitPatch.Item2.Value.MapPatchPointX, oExitPatch.Item2.Value.MapPatchPointY));
