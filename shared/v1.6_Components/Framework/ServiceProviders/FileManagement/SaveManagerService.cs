...
namespace SDV_Realty_Core.Framework.ServiceProviders.FileManagement
{
    internal class SaveManagerService : ISaveManagerService
    {
        ...
        {
            logger.Log($"Handling => SaveCreatedEventArgs",LogLevel.Debug);
            PostSave();
        ...
        public override void PreSave()
        {
            ...
            foreach (Action act in PreSaveHooks.OrderBy(p=>p.Item1).Select(p=>p.Item2))
            {
                logger.Log($"Firing PreSaveHook {act.Method.DeclaringType.Name}.{act.Method.Name}", LogLevel.Debug);
                act();
            }
        }
        /// <summary>
        ...
        {
#if DEBUG
            logger.Log("Firing PostSave event",LogLevel.Debug);
#endif
            //saveManager.PostSave();
            foreach (Action act in PostSaveHooks.OrderBy(p=>p.Item1).Select(p=>p.Item2))
            {
                logger.Log($"Firing PostSave {act.Method.DeclaringType.Name}.{act.Method.Name}", LogLevel.Debug);
                act();
            }
        }
        private void HandleGameLaunched(EventArgs e)
        {
            ...
            {
                logger.Log($"  {act.Method.DeclaringType.Name}.{act.Method.Name}", LogLevel.Debug);
            }
        ...
        {
#if DEBUG
            logger.Log($"Handling => SavingEventArgs", LogLevel.Debug);
#endif
            PreSave();
        }
        ...
        {
#if DEBUG
            logger.Log($"Handling => SaveCreatingEventArgs", LogLevel.Debug);
#endif
            PreSave();
