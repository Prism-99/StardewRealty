...
namespace SDV_Realty_Core.Framework.ServiceInterfaces.Utilities
{
    ...
    internal abstract class IUtilitiesService : IService
    {
        ...
        public void InvalidateCache(string cacheName, string? reason = null)
        {
            if (string.IsNullOrEmpty(cacheName)) return;

#if DEBUG
            if (!string.IsNullOrEmpty(reason))
                logger.Log($"Invalidating cache '{cacheName}': {reason}", LogLevel.Debug);
#endif
            try
            {
        ...
        /// <summary>
        /// Returns the client type of the current game instance.
        /// </summary>
        /// <returns>The client type of the current game instance.</returns>
        public GameInstanceTypes InstanceType()
        {
            if (Context.IsSplitScreen)
                return Context.IsMainPlayer ? GameInstanceTypes.SSHost : GameInstanceTypes.SSRemote;
            else if (Context.IsMultiplayer)
                return Context.IsMainPlayer ? GameInstanceTypes.MPHost : GameInstanceTypes.MPRemote;

            return GameInstanceTypes.SinglePlayer;
        }
            ...
            if (addLocation)
            {
                switch (locationType)
                {
                    ...
                    case LocationType.FarmExpansion:
                        location = new FarmExpansionLocation(map, assetPath, locationName, logger, this, ModDataService, locationTuner);
                        break;
                }
        ...
        internal void FixBuildingWarps(Building building, string expansionName)
        {
            ...
            else
            {
                ...
                {
                    if (warp.TargetName.Equals(baseFarmName) || warp.TargetName.Equals(expansionName))
                    {
