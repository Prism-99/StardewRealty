...
#if DEBUG
            logger.Log($"{locations.Count} Farm Expansions to Data/Locations", LogLevel.Debug);
#endif
            e.Edit(asset =>
            {
                ...
                foreach (var data in _expansionManager.expansionManager.LocationDataCache.Where(p=>locations.Where(q=>q.Name==p.Key).Any()))
                {
