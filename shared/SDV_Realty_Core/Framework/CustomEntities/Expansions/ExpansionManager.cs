...
namespace SDV_Realty_Core.Framework.Expansions
{
    internal class ExpansionManager
    {
        ...
        public bool ActivateExpansion(string expansionName, int gridId = -1, bool addToGrid = true)
        {
            ...
            //
            AddLocationDataDefinitionToCache(modDataService.farmExpansions[expansionName]);
        ...
        internal void AddLocationDataDefinitionToCache(FarmExpansionLocation location)
        {
            ...
            {
#if DEBUG
                logger.Log($"Add LocationCacheData for {location.DisplayName} ({location.Name})", LogLevel.Debug);
#endif
                ...
                if (createNewData)
                {
                    ...
                    //
#if DEBUG
                    logger.Log($"FishAreas: {contentPack?.FishAreas.Count??-1}", LogLevel.Debug);
#endif
        ...
        public bool LoadExpansionDetails(string expansionName)
        {
            ...
            if (modDataService.validContents.TryGetValue(expansionName, out ExpansionPack contentPack))
            {
                ...
                FarmExpansionLocation expansionLocation = new FarmExpansionLocation(modDataService.ExpansionMaps[mapName], mapPath, contentPack.LocationName, logger, expansionDetails, utilitiesService, modDataService, locationTunerIntegrationService)
                {
                    IsFarm = true,
                    IsOutdoors = true,
        ...
        internal void LoadLocationDefinitions(string loadContext)
        {
            ...
            //
            //  if run on the 2nd instance of SplitScreen
            //  duplicate entry error is thrown
            //
