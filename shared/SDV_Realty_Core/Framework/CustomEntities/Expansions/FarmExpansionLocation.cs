...
namespace SDV_Realty_Core.Framework.Expansions
{
    ...
    public class FarmExpansionLocation : GameLocation, ICloneable
    {
        ...
        public FarmExpansionLocation(string mapPath, string a) : base(GetConstructorMapPath(mapPath), a)
        {
            //logger = olog;
            //this.Map = omap;
        ...
        internal FarmExpansionLocation(Map omap, string mapPathValue, string locationName, ILoggerService olog, IUtilitiesService utilitiesService, IModDataService modDataService, ILocationTunerIntegrationService? locationTunerIntegrationService) : this()
        {
            logger = olog;
            Map = omap;
        ...
        internal FarmExpansionLocation(Map map, string mapPathValue, string locationName, ILoggerService olog, ExpansionDetails expansionDetails, IUtilitiesService utilitiesService, IModDataService modDataService, ILocationTunerIntegrationService locationTunerIntegrationService) : this(map, mapPathValue, locationName, olog, utilitiesService, modDataService, locationTunerIntegrationService)
        {
            logger = olog;
            this.expansionDetails = expansionDetails;
            ReloadExpansionDetails();
        }
        private static string GetConstructorMapPath(string path)
        {
            if (string.IsNullOrEmpty(path))
            {
                ModEntryHandler.logger.Log("FarmExpansionLocation, no path", LogLevel.Error);
            }
            return path;
        }
        ...
        /// <summary>
        /// Override base getFish method to add passive debugging
        /// and optional custom getFish method support.
        /// </summary>
        ...
        /// <param name="bobberTile"></param>
        /// <param name="locationName"></param>
        /// <returns>Fish Item of fish caught.</returns>
        ...
        {
            Item? fish = null;
            // flag to switch between base method and local method
            bool useBase = true;

            if (useBase)
            {
#if DEBUG
                logger.Log("getFish using base Method", LogLevel.Debug);
#endif
                fish = base.getFish(millisecondsAfterNibble, bait, waterDepth, who, baitPotency, bobberTile, locationName);
            }
            else
            {
#if DEBUG
                logger.Log("getFish using base local Method", LogLevel.Debug);
#endif
                if (locationName != null && locationName != Name && (!(locationName == "UndergroundMine") || !(this is MineShaft)))
                {
                    ...
                    {
                        fish = locationFromName.getFish(millisecondsAfterNibble, bait, waterDepth, who, baitPotency, bobberTile);
                    }
                }
                else
                {
                    if (bobberTile != Vector2.Zero && who.currentLocation?.NameOrUniqueName == NameOrUniqueName)
                    {
                        foreach (Building building in buildings)
                        {
                            ...
                            {
#if DEBUG
                                logger.Log($"Fishing in pond", LogLevel.Debug, 1);
#endif
                                fish = fishPond.CatchFish();
                            }
                        }
                    }
                    else
                    {
                        ...
                        {
#if DEBUG
                            logger.Log($"fishFrenzy: {fishFrenzyFish.Value}", LogLevel.Debug, 1);
#endif
                            fish = ItemRegistry.Create(fishFrenzyFish.Value);
                        }
                        else
                        {
                            bool isTutorialCatch = who.fishCaught.Length == 0;
                            fish = GetFishFromLocationData_Local(Name, bobberTile, waterDepth, who, isTutorialCatch, isInherited: false, this, null) ?? ItemRegistry.Create("(O)168");
                        }
                    }
                }
            }
#if DEBUG
            logger.Log($"Fish on hook: {fish?.DisplayName ?? "None"}", LogLevel.Info);
#endif

            return fish ?? ItemRegistry.Create("(O)145");
        }
        internal Item GetFishFromLocationData_Local(string locationName, Vector2 bobberTile, int waterDepth, Farmer player, bool isTutorialCatch, bool isInherited, GameLocation location, ItemQueryContext itemQueryContext)
        {
            if (location == null)
            {
...
#if DEBUG
            logger.Log($"Fish season: {seasonForLocation}", LogLevel.Debug, 1);
            logger.Log($"FishAreaId: {id ?? "null - Default"}", LogLevel.Debug, 1);
#endif
            bool flag = false;
            ...
            if (player?.CurrentTool is FishingRod fishingRod && fishingRod.isFishing)
            {
                ...
                SDObject bait = fishingRod.GetBait();
                if (bait != null)
                {
            ...
            //IEnumerable<SpawnFishData> enumerable = locationData.Fish;// = Game1.locationData["Default"].Fish;
...
#if DEBUG
            logger.Log($"  fishList: {string.Join(", ", enumerable.Select(p => p.ItemId).ToList())}", LogLevel.Debug, 1);
#endif
            int num = 0;
            ...
            for (int i = 0; i < 2; i++)
            {
                foreach (SpawnFishData spawn in enumerable)
                {
                    ...
                    float chance = spawn.GetChance(hasCuriosityLure, player.DailyLuck, player.LuckLevel, (float value, IList<QuantityModifier> modifiers, QuantityModifier.QuantityModifierMode mode) => Utility.ApplyQuantityModifiers(value, modifiers, mode, location), spawn.ItemId == text);
                    ...
                    {
                        logger.Log($"Location '{location?.NameOrUniqueName}' failed parsing item query '{query}' for fish '{spawn.Id}': {error}", LogLevel.Error, 1);
                    });
                    if (item2 == null)
                    {
                        continue;
                    ...
                    Item item3 = item2;
                    if ((spawn.CatchLimit <= -1 || !player.fishCaught.TryGetValue(item3.QualifiedItemId, out var value2) || value2[0] < spawn.CatchLimit) && CheckGenericFishRequirements(item3, allFishData, location, player, spawn, waterDepth, flag, hasCuriosityLure, spawn.ItemId == text, isTutorialCatch))
                    {
        ...
        //public override bool TryGetFishAreaForTile(Vector2 tile, out string id, out FishAreaData data)
        //{
