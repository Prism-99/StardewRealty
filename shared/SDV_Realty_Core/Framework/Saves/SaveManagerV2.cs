...
namespace SDV_Realty_Core.Framework.Saves
{
    internal class SaveManagerV2
    {
        ...
        {
            logger.Log("SaveManager => GameLoop_SaveCreating", LogLevel.Debug);
            //
        ...
        private void MigrateSaveFiles(string destinationDirectoy)
        {
            ...
            //
            logger.Log($"Migrating Stardew Realty data directory", LogLevel.Info,1);
        ...
        /// <summary>
        /// Load acutaual data file and add contents to game
        /// </summary>
        /// <param name="filename"></param>
        /// <param name="loadOnly"></param>
        /// <returns></returns>
        private bool Load(string filename, bool loadOnly = false)
        {
            //
            ...
            //
            logger?.Log("SaveManager => Load", LogLevel.Debug);
            ...
            else if (save_V1_6.BackupExists(GetDataDirectoryLocation(), out string backupPath))
            {
                ...
                {
                    logger.Log("Backup file used.", LogLevel.Info,1);
                    SaveLoaded();
                    return true;
                }
            }
        ...
        /// <summary>
        /// SDR data file has been loaded into the game.
        /// Trigger any processes that need to be run.
        /// </summary>
        private void SaveLoaded()
        {
            ...
            //
            //  re-generate location/data cache
            //
        ...
        {
            logger.Log("SaveManager => PostSave", LogLevel.Debug);
            expansionManager.expansionManager.ReAddActiveExpansions();
        }
        internal void PreSave()
        {
            ...
            logger.Log($"SaveManager.PreSave.  {modDataService.farmExpansions.Count} Expansion records.", LogLevel.Debug, 1);
            ...
            foreach (FarmExpansionLocation expansionLocation in modDataService.farmExpansions.Values)
            {
                try
                {
                    ...
                    {
                        logger.Log($"Removing expansion {expansionLocation.Name} from save", LogLevel.Debug,1);
            ...
            foreach (GameLocation gameLocation in Game1.locations.ToList())
            {
                ...
                {
                    logger?.Log($"FarmExpansionLocation {farmExpansion.Name} did not get cleared.", LogLevel.Debug,1);
                }
                else if (gameLocation is LocationExpansion locationExpansion)
                {
                    logger?.Log($"LocationExpansion {locationExpansion.Name} did not get cleared.", LogLevel.Debug, 1);
                ...
                if (gameLocation.modData.ContainsKey(IModDataKeysService.FELocationName))
                {
                    ...
                    //
                    logger.Log($"Removing custom interior {gameLocation.Name}", LogLevel.Debug, 1);
        ...
        internal void DoDayStartedTasks(EventArgs e)
        {
            ...
            logger.Log("SaveManager => DoDayStartedTasks", LogLevel.Debug);
            //
        ...
        {
            logger.Log("SaveManager => SaveFile", LogLevel.Debug);
            ...
            {
                logger.Log($"Farm Expansions saved to {dataSaver.GetSaveFilename(GetDataDirectoryLocation())}", LogLevel.Debug,1);
                return;
            }
            //
        ...
        //}
        /// <summary>
        /// Loads the SDR data file into the game.
        /// </summary>
        ...
        internal bool LoadSaveFile(string loadContext, string saveFilename = null, bool loadOnly = false)
        {
            ...
            logger.Log($"SaveManager => LoadSaveFile called from {loadContext}", LogLevel.Debug,1);
            ...
            if (File.Exists(saveFilePath))
            {
                ...
                try
                {
                    using (var reader = XmlReader.Create(saveFilePath))
                    {
                        ...
                        {
                            logger.Log($"Found custom buildings in Farm, {baseBuildings[Game1.getFarm().Name].buildings.Count}", LogLevel.Debug, 1);
                ...
                {
                    logger.LogError("SaveManagerV2.LoadBaseFarmData", ex);
                }
            }
        ...
        private void LoadActiveExpansions()
        {
            ...
            if (ExpansionLoaded)
            {
                logger?.Log("Skipping restore expansions", LogLevel.Debug,1);
            }
            else
            {
                logger.Log("Restoring expansions", LogLevel.Debug,1);
                ...
                foreach (string expansionKey in modDataService.farmExpansions.Keys)
                {
                    ...
                    if (expansionLocation.Active)
                    {
                        ...
                        //  game
                        //
                        logger.Log($"Restoring {expansionLocation.Name}", LogLevel.Debug,2);
