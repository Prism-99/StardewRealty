...
namespace SDV_Realty_Core.Framework.Saves
{
    ...
    public class Save_V1_6 : ISDRSave
    {
        ...
        private void LogSaveContents(SDict saveData)
        {
            logger.Log("======================", LogLevel.Debug);
            logger.Log("= SDR Save Contents  =", LogLevel.Debug);
            logger.Log("======================", LogLevel.Debug);
            logger.Log($"Total # of Expansions in the save: {saveData.Count}", LogLevel.Debug);
            logger.Log($"Active|Grid Id|Expansion Key", LogLevel.Debug);
            logger.Log($"------------------------------------------------", LogLevel.Debug);
            foreach (var expansion in saveData.OrderBy(p => p.Value.GridId))
            {
                logger.Log($"  {(expansion.Value.Active ? "Y" : "N")}        {expansion.Value.GridId}  {expansion.Key}", LogLevel.Debug);
            }
            logger.Log($"------------------------------------------------", LogLevel.Debug);
        }
        /// <summary>
                    ...
                    try
                    {
                        ...
                        if (addSaveExpansion)
                        {
                            ...
                            if (modDataService.farmExpansions.TryGetValue(saveExpansionKey, out FarmExpansionLocation expansionLocation))
                            {
                                ...
                                // bug fix for error in 2.0.4
                                expansionLocation.Active = true;// saveData[saveExpansionKey].Active;
                                ...
                                expansionLocation.BaseBuildingsAdded = true;
                                expansionLocation.buildings.Clear();
                                foreach (Building building in saveData[saveExpansionKey].buildings.ToList())
                                {
                                    //
                                    //  fix for double adding starter buildings
                                    //
                                    //  only stardew meadows needs checking
                                    //
                                    if (expansionLocation.Name == "prism99.sdr.MeadowlandsFarm")
                                    {
                                        var bldings = saveData[saveExpansionKey].buildings.Where(p => p.tileX == building.tileX && p.tileY == building.tileY);
                                        if (bldings.Count() > 1)
                                        {
                                            saveData[saveExpansionKey].buildings.Remove(bldings.First());
                                            logger.Log($"Fixed duplicate auto added building", LogLevel.Info);
                                            continue;
                                        }
                                    }
