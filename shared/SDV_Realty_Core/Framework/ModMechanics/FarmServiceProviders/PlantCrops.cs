...
namespace SDV_Realty_Core.Framework.ModMechanics.FarmServiceProviders
{
    ...
    internal class PlantCrops : IFarmServiceProvider
    {
        ...
        public PlantCrops(IModDataService modDataService, IUtilitiesService utilities, ILoggerService loggerService) : base(modDataService, utilities, loggerService)
        {
            serviceCharge = 0.1f;
        }
        ...
        private void PickCrop(string expansionNaame)
        {
            ...
            if (location != null)
            {
                foreach (var crop in crops.OrderBy(p => TokenParser.ParseText(Game1.objectData[p.Value.GetCleanHarvestId()].DisplayName)))
                {
                    cropList.Add(new KeyValuePair<string, string>(crop.Key, I18n.PC_Quote(TokenParser.ParseText(Game1.objectData[crop.Value.GetCleanHarvestId()].DisplayName), utilitiesService.GetSeedPrice(crop.Key), Game1.objectData[crop.Value.GetCleanHarvestId()].Price * location.Map.DisplayWidth / Game1.tileSize * location.Map.DisplayHeight / Game1.tileSize)));
                }
